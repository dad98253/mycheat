				...
				if (data) {
					...
					if (name_start) {
					    ...
					    if ( minWordSize > maxWordSize ) {
					      	FCGI_printf("<h1> ///// ERROR : The minimum word size cannot be greater than the maximum word size.</h1>\n");
					        exit(EXIT_FAILURE);
						}
					    ...
					    for (long unsigned int m=0;m<WordListSize;m++){
					        ...
					        int correct = aspell_speller_check(speller, Wordlist[m], strlen(Wordlist[m]));
					        if (correct) {
					        	FCGI_printf("<h1>%s</h1>\n", Wordlist[m]);
					        }
