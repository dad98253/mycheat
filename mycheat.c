...
int main (void) {
    ...
    while (FCGI_Accept() >= 0) {
        ...
        FCGI_printf("<meta name=\"viewport\" content=\"width=device-width, initial-scale=1.0\">\n");
	FCGI_printf("<style>\r\n");
	FCGI_printf(" html {font-size: 18px;\n}\n");
// Adjust base font size for screens up to 768px wide 
	FCGI_printf("@media screen and (max-width: 768px) {\n html {\n   font-size: 16px;\n   }\n}\r\n");
// Further adjust base font size for screens up to 480px wide (e.g., mobile) 
	FCGI_printf("@media screen and (max-width: 480px) {\n html {\n   font-size: 14px;\n   }\n}\r\n");
	FCGI_printf(" body {font-family: Arial, sans-serif;\n font-size: 1rem;\n}\n");
	FCGI_printf(" h1 {font-size: 2.5rem;\nmargin-top: .05em;\nmargin-bottom: .05em;\n }\n");
            ...
            if (data) {
                ...
                if (name_start) {
                    ...
                    if( (strlen(letters) < 3) || (strlen(letters) > 8) ) {
                        ...
                        FCGI_printf("<h1>Bad input: the number of letters must be between 3 and 8</h1>\n");
                        exit(EXIT_FAILURE);
                    }


                    sprintf(command, "permute -m 3 %s 2>/dev/null |sort|uniq",letters);
...
//                        FCGI_printf("<h1>COMMAND: %s</h1>\n", command);
     ...
     while (fgets(buffer, sizeof(buffer), fp) != NULL) {
        ...
        if (line_len>1) { if (*(buffer+line_len-1) == 0xa ) *(buffer+line_len-1) = 0; } // strip off the line feed
...
//        output_string = realloc(output_string, output_len + line_len + 1);
        ...
        if (output_string == NULL) {
            ...
            FCGI_printf("<h1>Memory allocation failed</h1>\n");
            exit(EXIT_FAILURE);
        }
...
//        FCGI_printf("<h1>read %i characters: \"%s\"</h1>\n", line_len, output_string);
    ...
    if (i) {
        for (int j=0;j<i;j++) {
           ...
           sprintf(command, "echo \"%s\" | aspell list",output_string);
                   fp = popen(command, "r"); // "r" for reading the command's output
                    if (fp == NULL) {
                        perror("Failed to run command");
                        FCGI_printf("<h1>Failed to run %s command</h1>\n",command);
                        exit(EXIT_FAILURE);
                    }
                    else {
//                        FCGI_printf("<h1>COMMAND: %s</h1>\n", command);
...
//           FCGI_printf("<h1>%s</h1>", output_string);
        ...
        FCGI_printf("<h1>No output captured.</h1>\n");
                    ...
                    FCGI_printf("<h1>Letters not found in POST data.</h1>\n");
                }
                free(data);
            } else {
